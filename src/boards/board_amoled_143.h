...
// flashing the 1.75 build here gives a black screen and a silent I2C bus. In
// particular the 1.75's I2S audio pins (BCLK=9, DIN=10) collide with THIS board's
// QSPI CS=9 / SCLK=10, so audio.cpp/battery.cpp must stay gated on
// BOARD_HAS_AUDIO/BOARD_HAS_PMIC (see config.h) -- an ungated audio_begin() steals
// the display's QSPI pins right after display::begin() and blanks the screen.
//
// Ported from the aircraft "Capsule Radar" project's board_amoled_143.h (community
// contribution by @sgmess), which was confirmed on real hardware:
